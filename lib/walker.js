...
/* ------------------------------ Rules ------------------------------ */
Walker.rules = {
	Message: function(node, children)
	{
		var msg = merge( children );
		msg.raw = node.text();
		return msg
	},
	
	Prefix: function(node, children)
	{
		return children[0]
	},
	...
	{
		return {
			server: node.text().trim()
		}
	},

	Person: function(node, children)
	{
		return {
			person: merge(children)
		}
	},
	Nick: function(node)
	{
	...
	{
		return {
			user: node.text().slice(1).trim()
		}
	},
	Host: function(node)
	{
		return {
			host: node.text().slice(1).trim()
		}
	},
	...
	{
		return {
			command: node.text().toLowerCase()
		}
	},

	Params: function(node, children)
	{
		return {
			params: children
		}
