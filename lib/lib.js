...
function IRC(options)
{
  ...
  (this.options = options || {}).__proto__ = IRC.options
    ...
    function do_connect()
    {
        ...
        this.user(this.options.user.username, this.options.user.wallops, this.options.user.invisible, this.options.user.realname)
    ...
    function parseMessage(data)
    {
        ...
        {
            if (this.options.log)
                sys.puts("[RECV]" + buffer[i])
	    
	    console.client.send(buffer[i])
    ...
    **/
    this.privmsg = function(receiver, msg, protect)
    {
